...
class CBOWModel(nn.Module):
    ...
    """
     def __init__(self, vocab_size, embedding_dim, device, huffman_paths=None, negative_samples=5):
        """
     ...
     def forward(self, inputs, target, negative_samples):
        ...
        return positive_scores.to(self.device), negative_scores.to(self.device)
...
def collate_fn(batch):
    ...
    negative_samples = [neg.clone().detach() for neg in negative_samples]
    
    # 返回填充后的上下文信息序列、目标信息张量和负样本信息张量列表
    return padded_contexts, targets, negative_samples

def train(device, model, dataloader, num_epochs=100, learning_rate=0.001):
    """
    训练模型。
    ...
    for epoch in range(num_epochs):
        ...
        for context, target, negative_samples in dataloader:
            ...
            # 将负样本列表合并成一个二维张量，并移动到指定设备
            negative_samples = [neg.clone().detach().to(device) for neg in negative_samples]
            negative_samples = torch.stack(negative_samples, dim=0)
            negative_samples.to(device)
            # 零化梯度，为新一轮反向传播做准备
            optimizer.zero_grad()
            # 前向传播，获取正样本和负样本的分数
...
def train_cbow(file_path, output_file, embedding_dim=100, window_size=2, batch_size=32, num_epochs=100, learning_rate=0.001, chunk_size=10000, min_freq=1):
    ...
    # 训练模型
    trained_model = train(device, model, dataloader, num_epochs, learning_rate)
...
if __name__ == "__main__":
    ...
    output_file = 'G:\\nlp\\seg_jieba_1_1_skip_embeddings_1.txt'
    start = int(time.time())
    print("start time",start)
    # 训练 CBOW 模型并获取词向量
    train_cbow(file_path, embedding_dim=20, num_epochs=20, learning_rate = 0.01, output_file = output_file)
    end = int(time.time())
    print("end time", end)
    print("耗时", end - start )
